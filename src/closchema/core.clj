(ns closchema.core
  "This is JSON Schema in Clojure.
  See http://tools.ietf.org/html/draft-zyp-json-schema-02
  Main purposed is to allow object validation, but schema metadata can be used
  for exposing contracts as well."
...
(defmacro with-validation-context
  ...
  invalidations to be captured. This strategy removes the need of raising
  exceptions at every single invalid point, and allows context information to
  be used when reporting about errors. Nested contexts are just ignored."
  [& body]
  `(let [body# #(do ~@body
...
(defmacro walk-in
  "Step inside a relative path, from a previous object. This information is
...
(defmulti validate*
  "Dispatch on object type for validation. If not implemented, performs
  only basic type validation. Users can extend which types are supported
  by implementing validation for new types."
  (fn [schema instance]
    (if (:enum schema)
      "enum"
      (:type schema))))
...
(defn validate
  "Entry point. A validation context is created and
  validation is dispatched to the appropriated multimethod."
  [schema instance]
  (with-validation-context
...
(defmethod validate* "object"
  ...
  #_ "validate instance properties (using invidivual or additional schema)"
...
(defmethod validate* "enum"
  ...
  (if-not (true? (some #(= % instance) (schema :enum)))
    (invalid :value-not-in-enum
             {:enum (schema :enum) :value instance })))
...
(defmethod validate* "number"
  ...
  (when (schema :maximum)
   (if-not (> (schema :maximum) instance)
    (invalid :value-lower-them-maximum
             {:maximum (schema :maximum) :value instance })))

  (when (schema :minimum)
   (if-not (< (schema :minimum) instance)
    (invalid :value-lower-them-minimum
             {:minimum (schema :minimum) :value instance })))

  (when (schema :maximumCanEqual)
   (if-not (>= (schema :maximumCanEqual) instance)
    (invalid :value-lower-them-maximumCanEqual
             {:maximumCanEqual (schema :maximumCanEqual)
              :value instance })))

  (when (schema :minimumCanEqual)
   (if-not (<= (schema :minimumCanEqual) instance)
    (invalid :value-lower-them-minimumCanEqual
             {:minimumCanEqual (schema :minimumCanEqual) :value instance })))
